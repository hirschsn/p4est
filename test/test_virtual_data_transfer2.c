...
void setup_p4est_structs(sc_MPI_Comm mpicomm, const int level,
...
void free_p4est_structs(p4est_connectivity_t *conn, p4est_t *p4est,
                        p4est_ghost_t *ghost, p4est_mesh_t *mesh,
                        p4est_virtual_t *vq, p4est_virtual_ghost_t *vg) {
...
void perform_test_flat(p4est_connectivity_t *conn, p4est_t *p4est,
                       p4est_ghost_t *ghost, p4est_mesh_t *mesh,
                       p4est_virtual_t *vq, p4est_virtual_ghost_t *vg) {
  int qid, vid;
  int ctr = 0;
  ...
  int *flat_data_local = P4EST_ALLOC(int, lq + lv);
  int *flat_data_ghost = P4EST_ALLOC(int, gq + gv);

  for (qid = 0; qid < lq; ++qid) {
    flat_data_local[ctr] = qid;
    ++ctr;
  ...
  p4est_virtual_ghost_exchange_data(p4est, ghost, mesh, vq, vg, sizeof(int),
...
void populate_per_level_data(p4est_t *p4est, p4est_mesh_t *mesh,
                             p4est_virtual_t *vq, int **level_data_local,
                             int **level_data_ghost) {
  int qid, vid;
  p4est_quadrant_t *q;
  const p4est_locidx_t lq = vq->local_num_quadrants;
  ...
  for (qid = 0; qid < lq; ++qid) {
    ...
    level_data_local[q->level][vq->quad_qreal_offset[qid]] = qid;
    if (vq->virtual_qflags[qid] != -1) {
      for (vid = 0; vid < P4EST_CHILDREN; ++vid) {
...
void perform_test_level(p4est_connectivity_t *conn, p4est_t *p4est,
                        p4est_ghost_t *ghost, p4est_mesh_t *mesh,
                        p4est_virtual_t *vq, p4est_virtual_ghost_t *vg) {
  int level;
  ...
  for (level = 0; level < P4EST_QMAXLEVEL; ++level) {
    ...
    level_data_local[level] = P4EST_ALLOC(int, quads_per_level);
    quads_per_level =
        (mesh->ghost_level + level)->elem_count +
        P4EST_CHILDREN * (vq->virtual_glevels + level)->elem_count;
    level_data_ghost[level] = P4EST_ALLOC(int, quads_per_level);
  }

  populate_per_level_data(p4est, mesh, vq, level_data_local, level_data_ghost);

  for (level = 0; level < P4EST_QMAXLEVEL; ++level) {
    p4est_virtual_ghost_exchange_data_level(
        p4est, ghost, mesh, vq, vg, level, sizeof(int),
        (void **)level_data_local, (void **)level_data_ghost);
  }
...
void execute_tests(sc_MPI_Comm mpicomm) {
  ...
  setup_p4est_structs(mpicomm, level, &conn, &p4est, &ghost, &mesh, &virt_quads,
                      &virt_ghost);
