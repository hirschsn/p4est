                             ...
                             const double * values)
{
  ...
  /* write point position data */
  fprintf (vtufile, "        <DataArray type=\"%s\" Name=\"%s\""
           " format=\"%s\" NumberOfComponents=\"%i\">\n",
           P4EST_VTK_FLOAT_NAME, vector_name, P4EST_VTK_FORMAT_STRING, P4EST_DIM);

#ifdef P4EST_VTK_ASCII
  for (il = 0; il < P8EST_DIM * Ncells; il++) {
#ifdef P4EST_VTK_DOUBLES
    fprintf (vtufile, "     %24.16e\n", values[il]);
#else
    fprintf (vtufile, "          %16.8e\n", values[il]);
...
#else
  float_data = P4EST_ALLOC (P4EST_VTK_FLOAT_TYPE, P4EST_DIM * Ncells);
  for (il = 0; il < P4EST_DIM * Ncells; ++il) {
    float_data[il] = (P4EST_VTK_FLOAT_TYPE) values[il];
  }
   ...
   */
  retval = p4est_vtk_write_binary (vtufile, (char *) float_data,
                                   sizeof (*float_data) * P4EST_DIM * Ncells);
  fprintf (vtufile, "\n");
  if (retval) {
    P4EST_LERROR (P4EST_STRING "_vtk: Error encoding points\n");
    fclose (vtufile);
  ...
  if (fclose (vtufile)) {
    P4EST_LERROR (P4EST_STRING "_vtk: Error closing point vector\n");
    return -1;
  }
  vtufile = NULL;
  ...
  if (mpirank == 0) {
    ...
    if (fclose (pvtufile)) {
      P4EST_LERROR (P4EST_STRING
                    "_vtk: Error closing parallel point vector\n");
      return -1;
    }
  }
