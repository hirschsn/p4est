...
#ifndef P8EST_MESHITERATE_H
#define P8EST_MESHITERATE_H
...
#define P8EST_MESHITER_DONE 13

/** Specify which quadrants to traverse w.r.t. local and ghost quads
 */
typedef enum
{
  P8EST_TRAVERSE_LOCAL,
  P8EST_TRAVERSE_LOCALGHOST,
  P8EST_TRAVERSE_GHOST
}
p8est_meshiter_localghost_t;

/** Characterize which quadrants to traverse w.r.t. real and virtual quads
 */
typedef enum
{
  P8EST_TRAVERSE_REAL,
  P8EST_TRAVERSE_REALVIRTUAL,
  P8EST_TRAVERSE_VIRTUAL
}
p8est_meshiter_realvirt_t;
...
typedef enum
{
  ...
  P8EST_TRAVERSE_PARALLEL_BOUNDARY,
  P8EST_TRAVERSE_INNER
}
p8est_meshiter_parallelboundary_t;

/** An iterator based on the information stored in p8est_mesh
 ...
 * do {
 *   ret_val = p8est_mesh_iterator_next(iterator);
 *   function_for_current_cell (iterator);
 * } (while ret_val != P8EST_MESHITER_DONE);
 * In the function for the current cell neighbor entities may be accessed up to
 * the specified connect type \a btype.
 *
 * p4est, ghost and mesh hold pointers to the respective p8est structures from
 ...
 * call p8est_meshiter_next() to change subquadrants, the number of subquadrants
 * is counted here instead of the p8est_quadrant_t that contain the respective 8
 ...
 *   0 ..  5 neighbor is in direction of face f_i
 *   6 .. 17 neighbor is in direction of edge e_(i-6)
 *  18 .. 25 neighbor is in direction of corner c_(i-18)
 ...
 */
void                p8est_meshiter_destroy (p8est_meshiter_t * mesh_iter);

/** Move iterator one quad further
 * \param [in][out] mesh_iter  The mesh iterator to be moved
 * \returns   0 if all went well or P8EST_MESHITER_DONE if the last quad was
 *            already processed
 */
int                 p8est_meshiter_next (p8est_meshiter_t * mesh_iter);
 ...
 *                             be found:
 *                              0 ..  5 direction of face f_i
 *                              6 .. 17 direction of edge e_(i-6)
 *                             18 .. 25 direction of corner c_(i-18)
 */
int                 p8est_meshiter_set_neighbor_quad_info (p8est_meshiter_t *
                                                           mesh_iter,
                                                           int direction);
 ...
 */
int                 p8est_meshiter_get_current_storage_id (p8est_meshiter_t *
                                                           mesh_iter);
 ...
 * \param[in]       mesh_iter  The mesh iterator
 */
int                 p8est_meshiter_get_neighbor_storage_id (p8est_meshiter_t *
                                                            mesh_iter);

SC_EXTERN_C_END;

#endif /* P8EST_MESHITERATE_H */
